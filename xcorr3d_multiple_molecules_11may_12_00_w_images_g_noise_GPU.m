...
PSFgpuArraysm=gpuArray(PSFarraysm);
...
%     for jj=1:int8(rand()*maxspotnum)+1
    ...
    for i=1:PSFzframes % Guessing the appropriate PSF library frame
        PSF=(PSFgpuArraysm(:,:,i)); % The cropped PSF frame from the library
        gC2(:,:,i)=xcorr2(gFrame,PSF); % Find correlation image
    ...
    for i=1:PSFzframes
        [max_C2(i,:), imax(i,:)] = max(C2(:,:,i));
        [ymax_C2(i,:), yimax(i,:)] = max(C2(:,:,i),[],2);
    end
