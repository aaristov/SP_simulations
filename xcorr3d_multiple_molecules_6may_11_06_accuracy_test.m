...
% xcorr2 normalization applied to correct for extreme z amplitudes

% 6 may
% 10-45 try 100 points z correction table zcortable6may
...
%     hold on;
    ...
    % Apply threshold
    max_C2_tr_mask = max_C2 > max(max_C2(:))*.5;
    ymax_C2_tr_mask = ymax_C2 > max(ymax_C2(:))*.5;
...
%     figure(7),plot(PSFamp,'o');title('PSFamp (xind)');
%     figure(8),plot(yPSFamp,'o');title('yPSFamp (yind)');
    ...
    while max_PSFamp > xcorr_tr && (PSFamp(ind_PSFamp+1) > 0 || PSFamp(ind_PSFamp-1) > 0)
        ...
        %         z fine search
        [xcorr,ycorr,zcorr] = finesearch(max_C2,ymax_C2,xind,yind,zind,zcortable,PSFzrange,PSFzframes);
        %         foundarray(:,foundnum+1)=...
        %             [horcorind2coord(xind),horcorind2coord(yind),...
        %             vertcorind2coord(zind,PSFzframes,PSFzrange)];
        foundarray(:,foundnum+1)=...
            [xp,yp,zp,xcorr,ycorr,zcorr];
        foundnum=foundnum+1;
...
hold on;
for i=1:foundnum
    scatter3(foundarray(1,i),foundarray(2,i),foundarray(3,i),1000);
    scatter3(foundarray(4,i),foundarray(5,i),foundarray(6,i),100);
end

[histox,histoy,histoz] = gethistofromfoundarray(foundarray, 10);
figure(1);
