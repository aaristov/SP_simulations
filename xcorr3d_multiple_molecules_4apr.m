...
%     figure(3);imagesc(max_C2_tr);
%     figure(4);imagesc(ymax_C2_tr);
...
%         figure(8),plot(yPSFamp,'o');
    ...
    while max_PSFamp > xcorr_tr && (PSFamp(ind_PSFamp+1) > 0 || PSFamp(ind_PSFamp-1) > 0)
        ...
        yind = imax(zind,xind);
        zfound=finesearch(max_C2,ymax_C2,xind,yind,zind,zcorrtable3);
...
%             vertcorind2coord(zind,PSFzframes,PSFzrange)];
        foundarray(:,foundnum+1)=...
            ...
            zfound];
        foundnum=foundnum+1;
%         figure(5);scatter3(horcorind2coord(xind),horcorind2coord(yind),vertcorind2coord(zind,PSFzframes,PSFzrange),100);
        tmpmask=ones(1,576);
        ytmpmask=ones(1,576);
        for m=1:9
