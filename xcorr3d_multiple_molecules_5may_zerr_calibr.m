...
setarray=zeros(3,testlength);
zcortable=zeros(2,testlength);
...
%         zp=zrangeoffset+(rand()-1/2)*zrange; % Random zp is set in nm
        zp=zrangeoffset+(j-1-(testlength-1)/2)/(testlength-1)*zrange; % Random zp is set in nm
        xp=0; % random xp, yp is set in um.
        yp=0;
...
%         [zcorr] = vertcorind2coord(zind);
        ...
        ytmpmask=ones(1,576);
        for m=1:11
            tmpmask(1,xind+m-6)=0;
            ytmpmask(1,yind+m-6)=0;
        end
...
%         %         z fine search
%         [zcorr] = finesearch(max_C2,ymax_C2,xind,yind,zind,zcorrtable3);
%         
%         zerr=zcorr-zp;
%         
%         foundarray(:,foundnum+1)=...
...
setarray;
figure;
plot(zcortable(1,:),zcortable(2,:),'o');
% figure(5);
% hold on;
% for i=1:foundnum
